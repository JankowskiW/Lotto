...
public record DrawingTime (
    ...
    // If fromTime equals toTime then drawing is exactly at that time (only if timeInterval is equal to 0)
    LocalTime fromTime,
    LocalTime toTime,
    List<DayOfWeek> daysOfWeek
) {
    public static DrawingTimeBuilder builder() {
        return new ValidationBuilder();
    }

    private static class ValidationBuilder extends DrawingTimeBuilder {
        @Override
        public DrawingTime build() {
